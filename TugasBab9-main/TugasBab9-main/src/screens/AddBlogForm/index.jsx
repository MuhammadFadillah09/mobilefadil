...
const [fcmToken, setFcmToken] = useState(null);
  ...
  useEffect(() => {
    // Ambil token FCM saat komponen mount
    ...
    // Pasang listener token refresh
    const unsubscribe = messaging().onTokenRefresh(token => {
      console.log('🔄 FCM Token refreshed:', token);
      setFcmToken(token);
      // Bisa kirim token ini ke backend agar update
    });
...
const handleUpload = async () => {
  ...
  setTimeout(async () => {
    try {
      ...
      // ✅ Tampilkan notifikasi setelah berhasil
      ...
      await notifee.displayNotification({
        ...
        body: `Blog "${blogData.title}" uploaded successfully.`,
        android: {
          channelId,
          smallIcon: 'ic_launcher', // pastikan ini ada di res/mipmap
        },
      });

      console.log('Blog uploaded with delay!');
    } catch (error) {
      console.error(error);
    }
  }, delayInSeconds * 1000);
    ...
    <View style={styles.container}>
      ...
      <ScrollView
        contentContainerStyle={{
          ...
          {/* <Text style={{fontSize: 12, color: colors.grey()}}>FCM Token:</Text>
        <Text
          style={{
            fontSize: 10,
            color: colors.black(),
